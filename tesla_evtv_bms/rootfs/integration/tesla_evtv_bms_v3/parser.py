...
def parse_udp_packet(payload: bytes, port: int) -> dict:
    _LOGGER.debug(f"[tesla_evtv_bms_v3] Received UDP payload on port {port}: {payload.hex()} (length={len(payload)})")

    if len(payload) < 12:
        _LOGGER.warning(f"[tesla_evtv_bms_v3] Ignored short packet on port {port} (length={len(payload)})")
        return None

    can_id = payload[8] + (payload[9] << 8) + (payload[10] << 16) + (payload[11] << 24)
    _LOGGER.debug(f"[tesla_evtv_bms_v3] Parsed CAN ID: {hex(can_id)}")

    if can_id not in [0x150, 0x151, 0x650, 0x651]:
        _LOGGER.debug(f"[tesla_evtv_bms_v3] Ignored unrecognized CAN ID: {hex(can_id)}")
        return None
